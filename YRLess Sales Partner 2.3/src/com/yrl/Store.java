...
public class Store implements Comparable<Store> {
	...
	public double getTotalMoney() {
		...
		for (int i = 0; i < sales.size(); i++) {
			...
			for (int j = 0; j < sales.get(i).getItems().size(); j++) {
				double taxes = roundToTwoDecimals(sales.get(i).getItems().get(j).getCost());
				double cost = roundToTwoDecimals(sales.get(i).getItems().get(j).getTaxes());
	...
	public int compareTo(Store o) {
		return this.getStoreCode().compareTo(o.getStoreCode());
	}
