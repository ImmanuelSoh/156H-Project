...
public class DataLoader {
	...
	public static Map<String, Item> loadItemsSQL(Map<String, Person> personsMap) {
		...
		try {
			...
			String query = "SELECT i.code, i.type, i.name, i.baseCost, " + "si.beginningLeaseDate, si.endingLeaseDate, "
...
//					Item addedItem = itemsMap.get(code);
	...
	public static void loadSaleItemsSQL(Map<String, Person> personsMap, Map<String, Item> itemsMap,
		...
		try {
			conn = DriverManager.getConnection(DatabaseInfo.URL, DatabaseInfo.USERNAME, DatabaseInfo.PASSWORD);
			String query = "SELECT s.saleCode, i.code AS itemCode FROM SaleItem si JOIN Sale s ON si.saleId = s.saleId JOIN Item i ON i.itemId = si.itemId;";
